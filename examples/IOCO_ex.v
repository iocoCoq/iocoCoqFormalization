...
(* "mais correto"
  ...
  unfold ind_s_after_IOLTS in H1. simpl in H1. unfold ind_s_after in H1.
  apply H1 in H5_1.
  assert(H': forall s : state,
    ind_s_seq_reachability 1 t s (create_s_IOLTS spec_s1_IOLTS) ->
    exists so' : set s_label,
       ind_out_one_state s so' (create_s_IOLTS spec_s1_IOLTS) /\
       (forall o : s_label, In o so' -> In o out_s)).
  { intros s'' H''. apply H4. apply H2. apply H''. }
  clear H1 H2 H3 H4.
  induction t as [| a t' IHt']; [| destruct a].
  - (* t = [] *)
    expand_s_seq_reachability H5_1; expand_out_one_state H5_2 x H5_3. clear H.
    assert (H: ind_s_seq_reachability 1 [] 1 (create_s_IOLTS spec_s1_IOLTS)).
    { apply s_seq_reachability_r1. apply empty_reachability_r1. }
    apply H' in H.
    destruct H as [so [H_1 H_2]]. apply H_2. inversion H_1; [elem_in_list |].
  ...
  - (* t = s_event "a" :: t' => destruct t' *)
    ...
    + (* t = [ s_event "a" ] *)
      expand_s_seq_reachability H.
      expand_s_seq_reachability H5_1; expand_out_one_state H5_2 x H5_3.
      clear H_so H5_1 H5_2 H H0 H1 H4.
      assert (H: ind_s_seq_reachability 1 [s_event "a"] 2 (create_s_IOLTS spec_s1_IOLTS)).
      { eapply s_seq_reachability_r2.
        ...
        - apply s_seq_reachability_r1. apply empty_reachability_r1. }
      apply H' in H.
      destruct H as [so [H_1 H_2]]. apply H_2. inversion H_1; [ expand_In H |].
      apply H1. exists 3. split; try apply transition_r1; elem_in_list.
    + (* t = s_event "a" :: s_event "x" :: t'' => induction t'' *)
      induction t'' as [| a t''' IHt''']; [| destruct a].
      * (* t = [ s_event "a"; s_event "x" ] *)
        expand_s_seq_reachability H.
        ...
        assert (H:
          ind_s_seq_reachability
            1
            [s_event "a"; s_event "x"]
            3
            (create_s_IOLTS spec_s1_IOLTS)).
        { eapply s_seq_reachability_r2.
          - apply one_step_reachability_r1 with (s1 := 1) (s2 := 2);
            try apply empty_reachability_r1. apply transition_r1. elem_in_list.
          - eapply s_seq_reachability_r2.
      ...
      * (* t = s_event "a" :: s_event "x" :: delta :: t''' => use IHt''' *)
        ...
        -- inversion H5_1. subst. expand_one_step_reachability H3;
           ...
           inversion H6; subst; expand_one_step_reachability H5;
           eapply s_seq_reachability_r2; try (apply H5); inversion H9; auto.
        -- intros s'' H''. apply H'. inversion H''.
           subst. expand_one_step_reachability H3.
           apply s_seq_reachability_r2 with (si := 2); auto.
           inversion H6. subst. expand_one_step_reachability H5.
           apply s_seq_reachability_r2 with (si := 3); auto.
           apply s_seq_reachability_r3; auto. elem_in_list.
    + (* t = s_event "a" :: delta :: t'' => contradiction *)
      expand_s_seq_reachability H.
  - (* t = delta :: t' => use IHt' *)
    inversion H5_1. subst. inversion H. subst. apply IHt'; auto.
    intros s'' H''. apply H'. apply s_seq_reachability_r3; auto.
Qed. *)
